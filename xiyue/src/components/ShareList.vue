 ...
 * @Author: your name
 * @Date: 2019-09-30 13:26:51
 * @LastEditTime: 2019-10-12 19:43:58
  ...
  <el-row class="sharelistBox">
    <div v-if="this.$route.name=='Share' && !this.$route.query.keywords" class="shareTitle">
      ...
      <ul v-if="sonclassList" class="shareclassTwo">
        <li v-for="(citem,index) in sonclassList"  :key='(citem,index)'>
          <a
    ...
    <el-col :span="24" class="s-item tcommonBox" v-for="(item,index) in articleList" :key="'article'+index">
      ...
      <header>
        ...
        <h2>
          ...
          <span
            v-html="showInitDate(item.create_time,'all')"
          >{{showInitDate(item.create_time,'all')}}</span> •
    ...
    showSearchShowList: function(initpage) {
      ...
      for (var i = 0; i < that.shareClass.length; i++) {
        if (that.classId == that.shareClass[i].class_id) {
          ...
          if (
            that.shareClass[i].ChildsSon && that.shareClass[i].ChildsSon.length > 0
          ) {
